...
void CDCServiceImpl::GetTabletListToPollForCDC(
  ...
  // This means the tablets of the new table have not been added to cdc_state table yet.
  if (std::find(table_ids.begin(), table_ids.end(), table_id) == table_ids.end()) {
    SetupErrorAndRespond(
        resp->mutable_error(),
        STATUS(NotFound, Format("Table not found under stream", req->table_info().table_id())),
