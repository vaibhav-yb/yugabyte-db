 ...
 public:
  ...
  Result<bool> Run() {
    ...
    state_.emplace_back(StateEntry {
      .key_entry = KeyBytes(),
      .write_time = VERIFY_RESULT(GetRootWriteTime()),
      .expiration = reader_.table_expiration_,
      .out = &result_,
    });

    if (reader_.projection_ && reader_.projection_->empty()) {
 ...
 private:
  ...
  Status Scan(CheckExistOnly check_exist_only) {
    while (reader_.iter_->valid()) {
      ...
      int compare_result; // cmp result between current subkeys and next requested column.
      for (;;) {
        if (check_exist_only || !reader_.projection_) {
          compare_result = 0;
        } else {
          compare_result = subkeys.compare_prefix(
  ...
  // Removes state_ elements that are not related to the current found entry.
  ...
  Result<bool> NextColumn() {
    if (!reader_.projection_) { // Used in tests only.
      return true;
    }
    ...
    if (last_found_ < static_cast<int64_t>(column_index_)) {
  ...
  Result<DocHybridTime> GetRootWriteTime() {
    ...
    if (!reader_.iter_->valid()) {
      return reader_.table_tombstone_time_;
    }
