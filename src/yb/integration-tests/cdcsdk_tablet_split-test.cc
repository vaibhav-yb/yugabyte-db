...
TEST_F(CDCSDKTabletSplitTest, YB_DISABLE_TEST_IN_TSAN(TestGetChangesAfterSplit)) {
  ANNOTATE_UNPROTECTED_WRITE(FLAGS_update_min_cdc_indices_interval_secs) = 1;
  ANNOTATE_UNPROTECTED_WRITE(FLAGS_cdc_state_checkpoint_update_interval_ms) = 1;
  ANNOTATE_UNPROTECTED_WRITE(FLAGS_aborted_intent_cleanup_ms) = 1000;
  ANNOTATE_UNPROTECTED_WRITE(FLAGS_cdc_populate_end_markers_transactions) = false;
  ANNOTATE_UNPROTECTED_WRITE(fLS::FLAGS_vmodule) = "cdc_service=1";
  google::protobuf::RepeatedPtrField<master::TabletLocationsPB> tablets;
  ASSERT_OK(SetUpWithParams(3, 1, false));
  const uint32_t num_tablets = 1;
  ...
  SleepFor(MonoDelta::FromMilliseconds(FLAGS_aborted_intent_cleanup_ms));
...
TEST_F(
  ...
  int64 received_records =
      ASSERT_RESULT(GetChangeRecordCount(stream_id, table, tablets, tablet_to_checkpoint, 200));
  ASSERT_GE(received_records, 200);
}
...
TEST_F(CDCSDKTabletSplitTest, YB_DISABLE_TEST_IN_TSAN(TestTabletSplitOnAddedTableForCDC)) {
  ...
  std::map<TabletId, CDCSDKCheckpointPB> tablet_to_checkpoint;
  int64 received_records =
      ASSERT_RESULT(GetChangeRecordCount(stream_id, table_2, tablets_2, tablet_to_checkpoint, 200));
...
TEST_F(
  ...
  int64 record_count =
      ASSERT_RESULT(GetChangeRecordCount(stream_id, table_2, tablets_2, tablet_to_checkpoint, 199));
...
TEST_F(CDCSDKTabletSplitTest, YB_DISABLE_TEST_IN_TSAN(TestSplitAfterSplit)) {
  ...
  ASSERT_GE(received_records, 400);
...
TEST_F(CDCSDKTabletSplitTest, YB_DISABLE_TEST_IN_TSAN(TestGetTabletListToPollForCDCWithTabletId)) {
  ...
  int64 record_count =
      ASSERT_RESULT(GetChangeRecordCount(stream_id, table, tablets, tablet_to_checkpoint, 199));
  ASSERT_GE(record_count, 199);
