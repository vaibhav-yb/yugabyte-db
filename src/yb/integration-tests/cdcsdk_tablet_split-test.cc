...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestGetChangesOnChildrenOnSplit)) {
  ...
  GetChangesResponsePB child_resp_1 = ASSERT_RESULT(GetChangesFromCDC(stream_id, tablets_after_split,
                                                        nullptr, 0, safe_hybrid_time, wal_segment_index,
                                                        false));

  GetChangesResponsePB child_resp_2 = ASSERT_RESULT(GetChangesFromCDC(stream_id, tablets_after_split,
                                                        nullptr, 1, safe_hybrid_time, wal_segment_index,
                                                        false));
}
