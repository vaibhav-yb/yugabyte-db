...
TEST_F(CDCSDKYsqlTest, TestLagMetricForUnconsumedSlot) {
  ...
  // Do not call GetChanges as we want to have the slot which is completely unconsumed.
  ...
  // As the table has become not of interest, and since this is the only table, the metric
  // entity will not be present anymore.
  ASSERT_OK(WaitFor(
      [&]() -> Result<bool> {
        auto metrics =
            VERIFY_RESULT(GetCDCSDKTabletMetrics(*cdc_service, tablets[0].tablet_id(), stream_id));
